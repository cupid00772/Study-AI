...
### 📌 二補數加法與溢位判斷 (Overflow) (對應 114年 Q9 - 官方更正為無正解)

**題目回顧：**

> In 8-bit two's-complement arithmetic, adding `0x73` + `0x94` raises the overflow flag because:
...
1. **判斷運算元的正負號 (在 8-bit 二補數系統中)**：
   - `0x73` (二進位：`0111 0011`)：最高位 (MSB) 為 0，是一個**正數** (十進位 +115)。
   - `0x94` (二進位：`1001 0100`)：最高位 (MSB) 為 1，是一個**負數** (十進位 -108)。
2. **判斷是否可能發生溢位 (Overflow)**：
   - **核心鐵律**：在二補數加法中，**「一正一負相加，絕對不可能發生溢位」**！因為兩個反號的數字相加，結果一定會落在兩者之間，絕對不會超出 8-bit 的表示範圍 (-128 ~ 127)。
   - 既然不可能發生溢位，題目的前提「raises the overflow flag (舉起了溢位旗標)」本身就是錯的。溢位旗標 (V flag) 根本沒有被舉起 (V=0)。因此，選項中所有的「原因」都無法成立，這也是為什麼官方最後將此題更正為「**無正確答案**」。
3. **實際二進位加法驗證與 Carry Flag (進位旗標) 釐清**：
   - `0111 0011` (`0x73`) + `1001 0100` (`0x94`) = `1 0000 0111`。
   - 8-bit 的運算結果為 `0000 0111` (`0x07`，十進位 +7)，這是正數，結果完全正確。
   - 進到最高位 (bit 7) 的 Carry-in 是 1，從最高位產生的 Carry-out 也是 1。
   - **Overflow (V) = Carry-in XOR Carry-out** = `1 XOR 1 = 0` (沒有溢位)。
   - **Carry (C) = Carry-out** = `1` (有產生進位)。

**選項錯誤分析：**

- **(A) Signs differ but result is negative**：錯誤。首先結果是正的 (`0x07`)；其次，正負號不同時相加永遠不會溢位。
- **(B) Both operands are positive but result is negative**：錯誤。`0x94` 是負數不是正數。而且結果是正數不是負數。*(註：如果題目出的是 `0x73` + `0x64`，這兩個都是正數且相加結果為 `0xD7` 負數，就會發生溢位，此時答案才會是 B。出題老師極有可能是想考這個觀念但數字設計錯誤。)*
- **(C) Carry-in to MSB = carry-out from MSB**：錯誤。這句話描述的是「Overflow = 0 (沒有發生溢位)」的條件。既然沒有發生溢位，這句話就不能當作「為什麼發生溢位」的理由。
- **(D) Parity bit = 0**：錯誤。同位元檢查碼與判斷溢位完全無關。
- **(E) Half-carry occurred**：錯誤。半進位 (bit 3 進到 bit 4) 是用於 BCD 運算的校正，與判斷是否發生溢位無關。

---

### 📌 演算法時間複雜度與實際 Benchmark 測量 (對應 114年 Q13)

**題目回顧：**

> You benchmark QuickSort and Measure that its running time on random permutations doubles when $n$ grows from $2 \mathrm{M}$ to $4 \mathrm{M}$. Which average-case complexity class is most consistent with this observation?
> (A) $O(n)$
> (B) $O(n \log \log n)$
> (C) $O(n \log n)$
> (D) $O(\sqrt{n})$
...
> [!WARNING]
> **陷阱警告：**
> 如果你選了 (A) $O(n)$，那就代表你被 "doubles" 這個詞騙了，忘記了 QuickSort 的本質。遇到這類題目，請先以「該演算法原本的理論複雜度」為基準，再去驗證題目的觀察是否合理，而不是單憑數字變化反推。

---

### 📌 C++ std::move() 與 Copy Elision (對應 114年 Q16)
...
2. **分析題目的程式碼 (賦值 Assignment vs 初始化 Initialization)**：
   - 題目明確寫出分兩行：
     ...
     v = std::move(foo());   // 第二行：賦值。這是一個「已經誕生」的變數要接收新資料。
     ```
   - **重點來了：Copy Elision (直接寫入記憶體的魔法) 只適用於「變數誕生 (初始化)」的那一刻！**
...
3. **選項分析與陷阱 (為什麼官方答案是 B？)**：
   ...
   - **結論**：老師把「呼叫端的賦值」跟「函式內部的回傳」搞混了，把所有「避免多餘搬移的魔法」統稱為 RVO，並且死背了「加上 std::move 就會破壞魔法」這個結論，才會給出 (B) 這個答案。
